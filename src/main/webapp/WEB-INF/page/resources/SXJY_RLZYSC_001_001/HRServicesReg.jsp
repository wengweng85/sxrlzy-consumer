            ...
            <div class="navbar-inner">
                <!--fluid 是偏移一部分-->
                    <div class="container-fluid " style="padding: 0px 20px;">
	                      ...
	                      <a class="btn btn-info " onclick="ef11Reg_save_data()"><i class="fa fa-save"></i>&nbsp;保存</a>
	                      </div>
                    </div>
           </div>
</div>
<div class="wrapper wrapper-content animated fadeInRight" id="query_form" >
     <form action="${contextpath}/resources/SXJY_RLZYSC_001_001/savereginfodata"  >
        <div id="input_content">
	            ...
	            <div class="ibox-content">
		            <div class="form-horizontal"  >
				       <div class="form-group">
				             <rc:textedit property="aef104" required="true" label="机构名称"   cols="2,4"
							  validate="{required:true,chinese:true,maxlength:200,messages:{required:'机构名称不能为空'}}" />
							 <rc:textedit property="aab998" required="true" label="统一社会信用代码"  cols="2,4"   
							  validate="{required:true,maxlength:18,messages:{required:'统一社会信用代码不能为空'}}" />
			           </div>
				       <div class="hr-line-dashed"></div>
				       <div class="form-group">
				       	     <rc:select property="aef105"  required="true" label="机构状态"  cols="2,2" codetype="AEF105"  
				       	       value="0"  onchange="jgzt(this)"  validate="{required:true,messages:{required:'机构状态不能为空'}}"/>
				       	     <rc:select property="aef106" required="true"  label="机构性质" cols="2,2"  codetype="AEF106" 
				       	     validate="{required:true,messages:{required:'机构性质不能为空'}}" />
				       	     <rc:select property="aef109" required="true" label="经营类型"  cols="2,2" codetype="AEF109"  
				       	     onchange="cFw(this)"  validate="{required:true,messages:{required:'经营类型不能为空'}}" />
				       </div>
				       <div class="hr-line-dashed" ></div>
					   <div class="form-group"   id="zy" style="border: 0px solid transparent">
                             <rc:textedit  label="经营范围" property="aef110_1" value="求职、用人登记；职业介绍、职业指导；收集、发布职业供求、培训信息；为家庭用工提供中介服务；互联网职业信息服务；法律、法规和政策咨询"  readonly="true"  required="true"  cols="2,10" />
                       </div>
				       ...
				       <div class="hr-line-dashed" style="display:none" id="line"></div>
				       <div class="form-group" id="xkzbh" style="display:none" >
				           <rc:textedit property="aef131" required="true" label="许可证编号"  cols="2,2"
							validate="{required:true,maxlength:12,messages:{required:'许可证编号不能为空'}}" />
				       </div>
			       </div>
		       </div>
	        </div>
	        ...
	        <div class="ibox ">
	            ...
	            <div class="ibox-content">
		            <div class="form-horizontal"  >
				       <div class="form-group">
			               <rc:textedit  property="aef121" required="true" label="机构联系人" cols="2,2" validate="{required:true,maxlength:50}" />
			               <rc:textedit  property="aef122" required="true"  label="机构联系人电话" cols="2,2" validate="{mobile:true,maxlength:20}"/>
	        ...
	        <div class="ibox ">
	            ...
	            <div class="ibox-content">
		            <div class="form-horizontal"  >
				        <div class="form-group">
			               <rc:textedit  property="aef125" required="true" label="法人代表姓名" cols="2,3" validate="{maxlength:50}" />
			               <rc:textedit  property="aef126" required="true"  label="法人代表身份证号" cols="2,3" validate="{idcard:true,maxlength:18}"/>
			            </div>
				        <div class="hr-line-dashed"></div>
				        <div class="form-group">
            ...
            <div id="toolbar" class="btn-group">
				 <button id="btn_user" type="button" class="btn btn-info"  onclick="hrservicePersonInfoAdd()">
				 <span class="glyphicon glyphicon-plus" aria-hidden="false"></span>新增 </button>
				 <button id="btn_delete" type="button" class="btn btn-danger" onclick="demo_delete_file()">
				 <span class="glyphicon glyphicon-remove" aria-hidden="false"></span>批量删除</button>
			</div>
            <div class="ibox-content">
			    <table id="ec11table" 
    ...
    $(function() {
    	...
    	var url="<c:url value='/resources/SXJY_RLZYSC_001_001/getEc11List'/>";
  ...
  //机构状态
		function jgzt(s){
		   if(s.value=="1")
		   {
			   ...
			   document.getElementById("xkzbh").style.display="";
			
		   }else {
			   document.getElementById("line").style.display="none";
			   document.getElementById("xkzbh").style.display="none";
		   }
		
		}
  
	//点击新增按钮触发
    function hrservicePersonInfoAdd(){
    	layer.open({
	   		  type: 2,
	   		  title: '机构工作人员新增页面',
	   		  shadeClose: false,
	   		  ...
	   		  shade: 0.8,
	   		  area: ['80%', '80%'],
	   		  content: "<c:url value='/resources/SXJY_RLZYSC_001_001/toaddPerson'/>" //iframe的url
		  });
     }	
    ...
    function view_details_info(eec111){
    	layer.open({
	   		  ...
	   		  shade: 0.8,
	   		  area: ['80%', '60%'],
	   		  content: "<c:url value='/resources/SXJY_RLZYSC_001_001/todetailPerson'/>/"+eec111  //iframe的url
 		  });
    }
    ...
    function hragencyPerson_edit_by_id(eec111,index){
    	layer.open({
	   		  ...
	   		  shade: 0.8,
	   		  area: ['80%', '80%'],
	   		  content: "<c:url value='/resources/SXJY_RLZYSC_001_001/toeditPerson'/>/"+eec111+"/"+index //iframe的url
    ...
    function demo_delete_file(){
	    	...
	    	if(ids.length>0){
	    		var layerindex=layer.confirm('确定批量删除这些数据吗,删除后将不可逆？', function(index){
    			  ...
    			  rc.ajax("<c:url value='/resources/SXJY_RLZYSC_001_001/batdeletePerson'/>", {selectnodes:ids.toString()},function (response) {
     	   		    	alert(response.message);
    ...
    function demo_delete_one(eec111){
    	var layerindex=layer.confirm('确定删除这些数据吗,删除后将不可逆？', function(index){
    		var url= "<c:url value='/resources/SXJY_RLZYSC_001_001/deletePerson/'/>"+eec111;
    		rc.ajax(url, null,function (response) {
	   		    	alert(response.message);
    	   		    $table.bootstrapTable('remove', {
	                    field: 'eec111',
