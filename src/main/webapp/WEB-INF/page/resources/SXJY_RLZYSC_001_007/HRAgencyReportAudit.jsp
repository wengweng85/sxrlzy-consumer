...
<body class="gray-bg">
    <div class="wrapper wrapper-content animated fadeInRight">
        ...
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-content">
			    <table id="ef11table" data-url="<c:url value='/resources/SXJY_RLZYSC_001_007/getEf11List'/>" 
    ...
    $('#ef11table').on('dbl-click-row.bs.table', function (e, row, $element) {
    	view_details_info(row.aab998,row.aee150);
    });  
    //操作编辑
    ...
    function hragency_detail_aef101(){
        ...
        if(selections.length>0){
            ...
            view_details_info(aab998,aee150);
        }else{
    ...
    //显示机构详细信息
    function view_details_info(aab998,aee150){
    	layer.open({
	   		  type: 2,
	   		  title: '机构详细信息页面',
	   		  shadeClose: false,
	   		  ...
	   		  shade: 0.8,
	   		  area: ['85%', '90%'],
	   		  content: "<c:url value='/resources/SXJY_RLZYSC_001_007/todetail'/>/"+aab998+"/"+aee150 //iframe的url
 		  });
    }
    //批量审核通过
    function hragency_reg_bataudit(){
		...
		layer.confirm('确定要批量审核所选数据吗？',function(index){
		   rc.ajax("<c:url value='/resources/SXJY_RLZYSC_001_007/bataudit?flag=1'/>", {selectnodes:selectnodes},function (response) {
    ...
    function hragency_reg_singleaudit(aab998,eae052){
		...
		layer.confirm('确定要审核该条记录吗？',function(index){
		   rc.ajax("<c:url value='/resources/SXJY_RLZYSC_001_007/singleaudit'/>", {aab998:aab998},function (response) {
		    	alert(response.message);
		    	$('#ef11table').refreshtable();
		   }); 
	    }); 
    ...
    function hragency_reg_notauditpass(aab998,eae052){
    	...
    	layer.open({
	   		  ...
	   		  shade: 0.8,
	   		  area: ['70%', '60%'],
	   		  content: "<c:url value='/resources/SXJY_RLZYSC_001_007/notauditpass'/>/"+aab998//iframe的url
		  });  
    }
    </script>
</body>
